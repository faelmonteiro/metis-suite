...
exibir_ajuda() {
  ...
  printf "${b}💻 GESTÃO & EXECUÇÃO DE COMANDOS${r}\n"
  printf "  ${g}%-18s${r} ${dim}│${r} %s\n" "/e [n]" "Executa o comando [n] no terminal sem fechar a conversa"
  printf "  ${g}%-18s${r} ${dim}│${r} %s\n" "![comando]" "Executa qualquer comando direto no terminal (ex: !ls -la)"
  printf "  ${g}%-18s${r} ${dim}│${r} %s\n" "/sh, /bash" "Abre um shell temporário (digite exit para voltar ao chat)"
  printf "  ${g}%-18s${r} ${dim}│${r} %s\n" "1, 2, 3..." "Seleciona e copia o comando [n] para o clipboard"
  printf "  ${g}%-18s${r} ${dim}│${r} %s\n\n" "/c [n]" "Copia o comando [n] para a área de transferência"

  printf "${c}💬 CHAT & NAVEGAÇÃO${r}\n"
  ...
  printf "  ${g}%-18s${r} ${dim}│${r} %s\n\n" "Esc / Ctrl+C / q" "Fecha o assistente e retorna ao terminal"
