...
# 1. Wayland Primary Selection (apenas se Wayland ativo)
if [ -n "$WAYLAND_DISPLAY" ] && command -v wl-paste >/dev/null 2>&1; then
  sel="$(wl-paste --primary --no-newline 2>/dev/null)"
fi

# 2. X11 Primary Selection (GNOME Terminal, Linux Mint, Kitty, etc.)
...
# 4. Fallback gravado pelo launcher ou seleção anterior
if [ -z "$sel" ] && [ -f "/tmp/qwen_selecao.txt" ] && [ -s "/tmp/qwen_selecao.txt" ]; then
